...
const packageBaseVersion = baseVersion(version)
...
async function dockerPush(push: boolean, tag: string) {
  if (push) {
...
async function publishImage(options: any, tags: string[], buildArgs: string[]) {
  ...
  for (const tag of otherTags) {
    ...
    await dockerPush(options.push, tag)
  }

  await dockerCommand(['tag', firstTag, lastTag])
  await dockerPush(options.push, lastTag)

  await dockerPush(options.push, firstTag)
}
...
async function buildHashedImage(
  ...
  const platform = `linux/${options.arch}`
  const platformTag = options.arch === 'amd64' ? '' : `-linux-${options.arch}`
  const lastTag = `${options.repo}:${name}${platformTag}`
  const baseVersionTag = `${lastTag}-${selectedVersion}${platformTag}`
  const firstTag = `${options.repo}:${name}-${await hashDockerfiles(name, baseImage)}${platformTag}`
  try {
    await dockerCommand(['pull', '--platform', platform, firstTag])
  } catch (e) {
    console.log(
...
async function dockerBuild(options: any) {
  const platform = `linux/${options.arch}`
  const platformTag = `linux-${options.arch}`
