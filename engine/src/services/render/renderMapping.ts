...
import { run as pngRun } from '../../commands/diagram/png.ts'
...
const generatePng = async (
  ...
  const file = Object.values(views).map((view: any) =>
    `${view.fileName}.drawio`
  ) as string[]
  await pngRun({ file, ...args })
}
...
const diagram = async (app: any, options: any, args: any) => {
  ...
  const renderView = async (viewString: string) => {
    ...
    if (!view) {
      ...
      if (
        viewString === 'all' &&
        !args.originalArgs.join(' ').includes('--view all')
      ) {
        view.disabled = true
      }
    }
  ...
  if (Object.values(views).length) {
    ...
    if (isDiagramPng) {
      await generatePng(options, args, views)
    }
  }
}
