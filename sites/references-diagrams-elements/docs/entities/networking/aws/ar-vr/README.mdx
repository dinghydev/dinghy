import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Aws AR & VR Entities


## AR &amp; VR

<Tabs>
  <TabItem value="Preview">

![](./ar-vr.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ArVr } from '@dinghy/diagrams/entitiesAwsArVr'

<ArVr/>

// or

import { extendStyle } from "@dinghy/base-components";
import { AR_VR } from '@dinghy/diagrams/entitiesAwsArVr'

<Shape 
  {...props}
  _style={extendStyle(props, AR_VR)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;points=[[0,0,0],[0.25,0,0],[0.5,0,0],[0.75,0,0],[1,0,0],[0,1,0],[0.25,1,0],[0.5,1,0],[0.75,1,0],[1,1,0],[0,0.25,0],[0,0.5,0],[0,0.75,0],[1,0.25,0],[1,0.5,0],[1,0.75,0]];outlineConnect=0;fontColor=#232F3E;gradientColor=#F34482;gradientDirection=north;fillColor=#BC1356;strokeColor=#ffffff;dashed=0;verticalLabelPosition=bottom;verticalAlign=top;align=center;html=1;fontSize=12;fontStyle=0;aspect=fixed;shape=mxgraph.aws4.resourceIcon;resIcon=mxgraph.aws4.ar_vr;',
  },
  _original_width: 78,
  _original_height: 78,

}
```

  </TabItem>
</Tabs>


## Sumerian

<Tabs>
  <TabItem value="Preview">

![](./sumerian.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Sumerian } from '@dinghy/diagrams/entitiesAwsArVr'

<Sumerian/>

// or

import { extendStyle } from "@dinghy/base-components";
import { SUMERIAN } from '@dinghy/diagrams/entitiesAwsArVr'

<Shape 
  {...props}
  _style={extendStyle(props, SUMERIAN)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;points=[[0,0,0],[0.25,0,0],[0.5,0,0],[0.75,0,0],[1,0,0],[0,1,0],[0.25,1,0],[0.5,1,0],[0.75,1,0],[1,1,0],[0,0.25,0],[0,0.5,0],[0,0.75,0],[1,0.25,0],[1,0.5,0],[1,0.75,0]];outlineConnect=0;fontColor=#232F3E;gradientColor=#F34482;gradientDirection=north;fillColor=#BC1356;strokeColor=#ffffff;dashed=0;verticalLabelPosition=bottom;verticalAlign=top;align=center;html=1;fontSize=12;fontStyle=0;aspect=fixed;shape=mxgraph.aws4.resourceIcon;resIcon=mxgraph.aws4.sumerian;',
  },
  _original_width: 78,
  _original_height: 78,

}
```

  </TabItem>
</Tabs>

