import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp Icons Management Tools Entities


## Anthos Service Mesh

<Tabs>
  <TabItem value="Preview">

![](./anthos-service-mesh.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { AnthosServiceMesh } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<AnthosServiceMesh/>

// or

import { extendStyle } from "@dinghy/base-components";
import { ANTHOS_SERVICE_MESH } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, ANTHOS_SERVICE_MESH)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>


## Cloud Billing API

<Tabs>
  <TabItem value="Preview">

![](./cloud-billing-api.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudBillingApi } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<CloudBillingApi/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_BILLING_API } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_BILLING_API)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>


## Cloud Console

<Tabs>
  <TabItem value="Preview">

![](./cloud-console.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudConsole } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<CloudConsole/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_CONSOLE } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_CONSOLE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 38,
}
```

  </TabItem>
</Tabs>


## Cloud Deployment Manager

<Tabs>
  <TabItem value="Preview">

![](./cloud-deployment-manager.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudDeploymentManager } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<CloudDeploymentManager/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_DEPLOYMENT_MANAGER } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_DEPLOYMENT_MANAGER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 28.000000000000004,
}
```

  </TabItem>
</Tabs>


## Cloud Logging

<Tabs>
  <TabItem value="Preview">

![](./cloud-logging.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudLogging } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<CloudLogging/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_LOGGING } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_LOGGING)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 38,
}
```

  </TabItem>
</Tabs>


## Cloud Shell

<Tabs>
  <TabItem value="Preview">

![](./cloud-shell.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudShell } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<CloudShell/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_SHELL } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_SHELL)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 38,
}
```

  </TabItem>
</Tabs>


## CloudAPIs

<Tabs>
  <TabItem value="Preview">

![](./cloudapis.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Cloudapis } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Cloudapis/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUDAPIS } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUDAPIS)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>


## Debugger

<Tabs>
  <TabItem value="Preview">

![](./debugger.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Debugger } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Debugger/>

// or

import { extendStyle } from "@dinghy/base-components";
import { DEBUGGER } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, DEBUGGER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 32,
  _height: 40,
}
```

  </TabItem>
</Tabs>


## Error Reporting

<Tabs>
  <TabItem value="Preview">

![](./error-reporting.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ErrorReporting } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<ErrorReporting/>

// or

import { extendStyle } from "@dinghy/base-components";
import { ERROR_REPORTING } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, ERROR_REPORTING)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>


## Profiler

<Tabs>
  <TabItem value="Preview">

![](./profiler.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Profiler } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Profiler/>

// or

import { extendStyle } from "@dinghy/base-components";
import { PROFILER } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, PROFILER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 34,
  _height: 40,
}
```

  </TabItem>
</Tabs>


## Stackdriver

<Tabs>
  <TabItem value="Preview">

![](./stackdriver.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Stackdriver } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Stackdriver/>

// or

import { extendStyle } from "@dinghy/base-components";
import { STACKDRIVER } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, STACKDRIVER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 34,
}
```

  </TabItem>
</Tabs>


## Trace

<Tabs>
  <TabItem value="Preview">

![](./trace.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Trace } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Trace/>

// or

import { extendStyle } from "@dinghy/base-components";
import { TRACE } from '@dinghy/diagrams/entitiesGcpIconsManagementTools'

<Shape 
  {...props}
  _style={extendStyle(props, TRACE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>

