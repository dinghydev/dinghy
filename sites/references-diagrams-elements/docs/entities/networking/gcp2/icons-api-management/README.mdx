import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp2 API Management Entities


## API
Monetization

<Tabs>
  <TabItem value="Preview">

![](./api-monetization.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ApiMonetization } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<ApiMonetization/>

// or

import { extendStyle } from "@dinghy/base-components";
import { API_MONETIZATION } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, API_MONETIZATION)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 42,
  _original_height: 42,

}
```

  </TabItem>
</Tabs>


## API Analytics

<Tabs>
  <TabItem value="Preview">

![](./api-analytics.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ApiAnalytics } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<ApiAnalytics/>

// or

import { extendStyle } from "@dinghy/base-components";
import { API_ANALYTICS } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, API_ANALYTICS)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 19,
}
```

  </TabItem>
</Tabs>


## Apigee
Sense

<Tabs>
  <TabItem value="Preview">

![](./apigee-sense.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ApigeeSense } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<ApigeeSense/>

// or

import { extendStyle } from "@dinghy/base-components";
import { APIGEE_SENSE } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, APIGEE_SENSE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 42,
  _original_height: 42,

}
```

  </TabItem>
</Tabs>


## Cloud
Endpoints

<Tabs>
  <TabItem value="Preview">

![](./cloud-endpoints.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudEndpoints } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<CloudEndpoints/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_ENDPOINTS } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_ENDPOINTS)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 25,
}
```

  </TabItem>
</Tabs>


## Developer
Portal

<Tabs>
  <TabItem value="Preview">

![](./developer-portal.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { DeveloperPortal } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<DeveloperPortal/>

// or

import { extendStyle } from "@dinghy/base-components";
import { DEVELOPER_PORTAL } from '@dinghy/diagrams/entitiesGcp2IconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, DEVELOPER_PORTAL)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 29,
}
```

  </TabItem>
</Tabs>

