import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp2 Hybrid and Multi Cloud Entities


## Anthos

<Tabs>
  <TabItem value="Preview">

![](./anthos.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Anthos } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<Anthos/>

// or

import { extendStyle } from "@dinghy/base-components";
import { ANTHOS } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<Shape 
  {...props}
  _style={extendStyle(props, ANTHOS)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 36,
}
```

  </TabItem>
</Tabs>


## Anthos Clusters

<Tabs>
  <TabItem value="Preview">

![](./anthos-clusters.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { AnthosClusters } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<AnthosClusters/>

// or

import { extendStyle } from "@dinghy/base-components";
import { ANTHOS_CLUSTERS } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<Shape 
  {...props}
  _style={extendStyle(props, ANTHOS_CLUSTERS)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 42,

}
```

  </TabItem>
</Tabs>


## Anthos Service
Mesh

<Tabs>
  <TabItem value="Preview">

![](./anthos-service-mesh.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { AnthosServiceMesh } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<AnthosServiceMesh/>

// or

import { extendStyle } from "@dinghy/base-components";
import { ANTHOS_SERVICE_MESH } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<Shape 
  {...props}
  _style={extendStyle(props, ANTHOS_SERVICE_MESH)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 41,
  _original_height: 42,

}
```

  </TabItem>
</Tabs>


## Stackdriver

<Tabs>
  <TabItem value="Preview">

![](./stackdriver.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Stackdriver } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<Stackdriver/>

// or

import { extendStyle } from "@dinghy/base-components";
import { STACKDRIVER } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<Shape 
  {...props}
  _style={extendStyle(props, STACKDRIVER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 36,
}
```

  </TabItem>
</Tabs>


## Traffic
Director

<Tabs>
  <TabItem value="Preview">

![](./traffic-director.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { TrafficDirector } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<TrafficDirector/>

// or

import { extendStyle } from "@dinghy/base-components";
import { TRAFFIC_DIRECTOR } from '@dinghy/diagrams/entitiesGcp2IconsHybridAndMultiCloud'

<Shape 
  {...props}
  _style={extendStyle(props, TRAFFIC_DIRECTOR)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 42,
  _original_height: 42,

}
```

  </TabItem>
</Tabs>

