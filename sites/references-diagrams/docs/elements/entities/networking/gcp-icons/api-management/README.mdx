import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp Icons API Management Entities


## API Analytics

<Tabs>
  <TabItem value="Preview">

![](./api-analytics.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ApiAnalytics } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<ApiAnalytics/>

// or

import { extendStyle } from "@dinghy/base-components";
import { API_ANALYTICS } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, API_ANALYTICS)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 18,
}
```

  </TabItem>
</Tabs>


## API Monetization

<Tabs>
  <TabItem value="Preview">

![](./api-monetization.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ApiMonetization } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<ApiMonetization/>

// or

import { extendStyle } from "@dinghy/base-components";
import { API_MONETIZATION } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, API_MONETIZATION)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>


## Apigee API Platform

<Tabs>
  <TabItem value="Preview">

![](./apigee-api-platform.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ApigeeApiPlatform } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<ApigeeApiPlatform/>

// or

import { extendStyle } from "@dinghy/base-components";
import { APIGEE_API_PLATFORM } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, APIGEE_API_PLATFORM)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>


## Apigee Sense

<Tabs>
  <TabItem value="Preview">

![](./apigee-sense.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ApigeeSense } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<ApigeeSense/>

// or

import { extendStyle } from "@dinghy/base-components";
import { APIGEE_SENSE } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, APIGEE_SENSE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>


## Cloud Endpoints

<Tabs>
  <TabItem value="Preview">

![](./cloud-endpoints.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudEndpoints } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<CloudEndpoints/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_ENDPOINTS } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_ENDPOINTS)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 24,
}
```

  </TabItem>
</Tabs>


## Developer Portal

<Tabs>
  <TabItem value="Preview">

![](./developer-portal.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { DeveloperPortal } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<DeveloperPortal/>

// or

import { extendStyle } from "@dinghy/base-components";
import { DEVELOPER_PORTAL } from '@dinghy/diagrams/entitiesGcpIconsApiManagement'

<Shape 
  {...props}
  _style={extendStyle(props, DEVELOPER_PORTAL)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 28.000000000000004,
}
```

  </TabItem>
</Tabs>

