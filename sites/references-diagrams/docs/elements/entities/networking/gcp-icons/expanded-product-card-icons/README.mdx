import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp Icons Expanded Product Card Icons Entities


## App Engine

<Tabs>
  <TabItem value="Preview">

![](./app-engine.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { AppEngine } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<AppEngine/>

// or

import { extendStyle } from "@dinghy/base-components";
import { APP_ENGINE } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, APP_ENGINE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 32,
  _height: 26,
}
```

  </TabItem>
</Tabs>


## Cloud Dataflow

<Tabs>
  <TabItem value="Preview">

![](./cloud-dataflow.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudDataflow } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<CloudDataflow/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_DATAFLOW } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_DATAFLOW)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 24,
  _height: 32,
}
```

  </TabItem>
</Tabs>


## Cloud DataProc

<Tabs>
  <TabItem value="Preview">

![](./cloud-dataproc.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudDataproc } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<CloudDataproc/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_DATAPROC } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_DATAPROC)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 32,
  _height: 30,
}
```

  </TabItem>
</Tabs>


## Compute Engine

<Tabs>
  <TabItem value="Preview">

![](./compute-engine.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ComputeEngine } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ComputeEngine/>

// or

import { extendStyle } from "@dinghy/base-components";
import { COMPUTE_ENGINE } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, COMPUTE_ENGINE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 32,
  _original_height: 32,

}
```

  </TabItem>
</Tabs>


## Container Engine

<Tabs>
  <TabItem value="Preview">

![](./container-engine.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ContainerEngine } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ContainerEngine/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CONTAINER_ENGINE } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, CONTAINER_ENGINE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 28.000000000000004,
  _height: 32,
}
```

  </TabItem>
</Tabs>


## Modifiers - Autoscaling

<Tabs>
  <TabItem value="Preview">

![](./modifiers-autoscaling.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersAutoscaling } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersAutoscaling/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_AUTOSCALING } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_AUTOSCALING)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 36,
  _original_height: 36,

}
```

  </TabItem>
</Tabs>


## Modifiers - Custom Virtual Machine

<Tabs>
  <TabItem value="Preview">

![](./modifiers-custom-virtual-machine.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersCustomVirtualMachine } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersCustomVirtualMachine/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_CUSTOM_VIRTUAL_MACHINE } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_CUSTOM_VIRTUAL_MACHINE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>


## Modifiers - High CPU Machine

<Tabs>
  <TabItem value="Preview">

![](./modifiers-high-cpu-machine.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersHighCpuMachine } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersHighCpuMachine/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_HIGH_CPU_MACHINE } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_HIGH_CPU_MACHINE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 34,
  _height: 30,
}
```

  </TabItem>
</Tabs>


## Modifiers - High Memory Machine

<Tabs>
  <TabItem value="Preview">

![](./modifiers-high-memory-machine.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersHighMemoryMachine } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersHighMemoryMachine/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_HIGH_MEMORY_MACHINE } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_HIGH_MEMORY_MACHINE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 30,
}
```

  </TabItem>
</Tabs>


## Modifiers - Preemptable VM

<Tabs>
  <TabItem value="Preview">

![](./modifiers-preemptable-vm.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersPreemptableVm } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersPreemptableVm/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_PREEMPTABLE_VM } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_PREEMPTABLE_VM)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 34,
  _height: 36,
}
```

  </TabItem>
</Tabs>


## Modifiers - Shared Core Machine F1

<Tabs>
  <TabItem value="Preview">

![](./modifiers-shared-core-machine-f1.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersSharedCoreMachineF1 } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersSharedCoreMachineF1/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_SHARED_CORE_MACHINE_F1 } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_SHARED_CORE_MACHINE_F1)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 36,
  _original_height: 36,

}
```

  </TabItem>
</Tabs>


## Modifiers - Shared Core Machine G1

<Tabs>
  <TabItem value="Preview">

![](./modifiers-shared-core-machine-g1.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersSharedCoreMachineG1 } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersSharedCoreMachineG1/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_SHARED_CORE_MACHINE_G1 } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_SHARED_CORE_MACHINE_G1)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 36,
  _original_height: 36,

}
```

  </TabItem>
</Tabs>


## Modifiers - Standard Machine

<Tabs>
  <TabItem value="Preview">

![](./modifiers-standard-machine.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersStandardMachine } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersStandardMachine/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_STANDARD_MACHINE } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_STANDARD_MACHINE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 36,
  _original_height: 36,

}
```

  </TabItem>
</Tabs>


## Modifiers - Storage

<Tabs>
  <TabItem value="Preview">

![](./modifiers-storage.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ModifiersStorage } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<ModifiersStorage/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MODIFIERS_STORAGE } from '@dinghy/diagrams/entitiesGcpIconsExpandedProductCardIcons'

<Shape 
  {...props}
  _style={extendStyle(props, MODIFIERS_STORAGE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 32,
}
```

  </TabItem>
</Tabs>

