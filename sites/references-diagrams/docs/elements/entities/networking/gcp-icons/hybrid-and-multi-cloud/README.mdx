import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp Icons Hybrid and Multi Cloud Entities


## Stackdriver

<Tabs>
  <TabItem value="Preview">

![](./stackdriver.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Stackdriver } from '@dinghy/diagrams/entitiesGcpIconsHybridAndMultiCloud'

<Stackdriver/>

// or

import { extendStyle } from "@dinghy/base-components";
import { STACKDRIVER } from '@dinghy/diagrams/entitiesGcpIconsHybridAndMultiCloud'

<Shape 
  {...props}
  _style={extendStyle(props, STACKDRIVER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 34,
}
```

  </TabItem>
</Tabs>


## Taffic Director

<Tabs>
  <TabItem value="Preview">

![](./taffic-director.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { TafficDirector } from '@dinghy/diagrams/entitiesGcpIconsHybridAndMultiCloud'

<TafficDirector/>

// or

import { extendStyle } from "@dinghy/base-components";
import { TAFFIC_DIRECTOR } from '@dinghy/diagrams/entitiesGcpIconsHybridAndMultiCloud'

<Shape 
  {...props}
  _style={extendStyle(props, TAFFIC_DIRECTOR)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 40,
  _original_height: 40,

}
```

  </TabItem>
</Tabs>

