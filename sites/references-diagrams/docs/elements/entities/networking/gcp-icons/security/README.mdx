import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp Icons Security Entities


## BeyondCorp

<Tabs>
  <TabItem value="Preview">

![](./beyondcorp.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Beyondcorp } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<Beyondcorp/>

// or

import { extendStyle } from "@dinghy/base-components";
import { BEYONDCORP } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, BEYONDCORP)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 38,
  _height: 40,
}
```

  </TabItem>
</Tabs>


## Cloud IAM

<Tabs>
  <TabItem value="Preview">

![](./cloud-iam.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudIam } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<CloudIam/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_IAM } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_IAM)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 32,
  _height: 40,
}
```

  </TabItem>
</Tabs>


## Cloud Security Scanner

<Tabs>
  <TabItem value="Preview">

![](./cloud-security-scanner.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudSecurityScanner } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<CloudSecurityScanner/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_SECURITY_SCANNER } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_SECURITY_SCANNER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 34,
}
```

  </TabItem>
</Tabs>


## Data Loss Prevention API

<Tabs>
  <TabItem value="Preview">

![](./data-loss-prevention-api.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { DataLossPreventionApi } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<DataLossPreventionApi/>

// or

import { extendStyle } from "@dinghy/base-components";
import { DATA_LOSS_PREVENTION_API } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, DATA_LOSS_PREVENTION_API)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 40,
  _height: 30,
}
```

  </TabItem>
</Tabs>


## Key Access Justifications

<Tabs>
  <TabItem value="Preview">

![](./key-access-justifications.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { KeyAccessJustifications } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<KeyAccessJustifications/>

// or

import { extendStyle } from "@dinghy/base-components";
import { KEY_ACCESS_JUSTIFICATIONS } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, KEY_ACCESS_JUSTIFICATIONS)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.st*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 38,
  _height: 40,
}
```

  </TabItem>
</Tabs>


## Key Management Service

<Tabs>
  <TabItem value="Preview">

![](./key-management-service.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { KeyManagementService } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<KeyManagementService/>

// or

import { extendStyle } from "@dinghy/base-components";
import { KEY_MANAGEMENT_SERVICE } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, KEY_MANAGEMENT_SERVICE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 32,
  _height: 40,
}
```

  </TabItem>
</Tabs>


## Security Command Center

<Tabs>
  <TabItem value="Preview">

![](./security-command-center.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { SecurityCommandCenter } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<SecurityCommandCenter/>

// or

import { extendStyle } from "@dinghy/base-components";
import { SECURITY_COMMAND_CENTER } from '@dinghy/diagrams/entitiesGcpIconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, SECURITY_COMMAND_CENTER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'editableCssRules=.*;html=1;shape=image;verticalLabelPosition=bottom;labelBackgroundColor=#ffffff;verticalAlign=top;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 34,
  _height: 40,
}
```

  </TabItem>
</Tabs>

