import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp2 Databases Entities


## AlloyDB

<Tabs>
  <TabItem value="Preview">

![](./alloydb.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Alloydb } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Alloydb/>

// or

import { extendStyle } from "@dinghy/base-components";
import { ALLOYDB } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, ALLOYDB)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 39,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## BigQuery

<Tabs>
  <TabItem value="Preview">

![](./bigquery.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Bigquery } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Bigquery/>

// or

import { extendStyle } from "@dinghy/base-components";
import { BIGQUERY } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, BIGQUERY)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 42,
  _original_height: 42,

}
```

  </TabItem>
</Tabs>


## Cloud
Bigtable

<Tabs>
  <TabItem value="Preview">

![](./cloud-bigtable.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudBigtable } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<CloudBigtable/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_BIGTABLE } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_BIGTABLE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 38,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Cloud
Spanner

<Tabs>
  <TabItem value="Preview">

![](./cloud-spanner.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudSpanner } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<CloudSpanner/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_SPANNER } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_SPANNER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 38,
}
```

  </TabItem>
</Tabs>


## Cloud
SQL

<Tabs>
  <TabItem value="Preview">

![](./cloud-sql.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudSql } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<CloudSql/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_SQL } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_SQL)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 32,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Database Migration
Service

<Tabs>
  <TabItem value="Preview">

![](./database-migration-service.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { DatabaseMigrationService } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<DatabaseMigrationService/>

// or

import { extendStyle } from "@dinghy/base-components";
import { DATABASE_MIGRATION_SERVICE } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, DATABASE_MIGRATION_SERVICE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 42,
  _original_height: 42,

}
```

  </TabItem>
</Tabs>


## Datastore

<Tabs>
  <TabItem value="Preview">

![](./datastore.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Datastore } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Datastore/>

// or

import { extendStyle } from "@dinghy/base-components";
import { DATASTORE } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, DATASTORE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 29,
}
```

  </TabItem>
</Tabs>


## Firestore

<Tabs>
  <TabItem value="Preview">

![](./firestore.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Firestore } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Firestore/>

// or

import { extendStyle } from "@dinghy/base-components";
import { FIRESTORE } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, FIRESTORE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 36,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Memorystore

<Tabs>
  <TabItem value="Preview">

![](./memorystore.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Memorystore } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Memorystore/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MEMORYSTORE } from '@dinghy/diagrams/entitiesGcp2IconsDatabases'

<Shape 
  {...props}
  _style={extendStyle(props, MEMORYSTORE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _original_width: 42,
  _original_height: 42,

}
```

  </TabItem>
</Tabs>

