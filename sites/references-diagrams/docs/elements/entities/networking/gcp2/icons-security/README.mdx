import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp2 Security Entities


## Chronicle

<Tabs>
  <TabItem value="Preview">

![](./chronicle.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Chronicle } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Chronicle/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CHRONICLE } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, CHRONICLE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 33,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Cloud
IAM

<Tabs>
  <TabItem value="Preview">

![](./cloud-iam.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudIam } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<CloudIam/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_IAM } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_IAM)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 34,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Cloud Key
Management

<Tabs>
  <TabItem value="Preview">

![](./cloud-key-management.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudKeyManagement } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<CloudKeyManagement/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_KEY_MANAGEMENT } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_KEY_MANAGEMENT)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 34,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Data Loss
Prevention API

<Tabs>
  <TabItem value="Preview">

![](./data-loss-prevention-api.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { DataLossPreventionApi } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<DataLossPreventionApi/>

// or

import { extendStyle } from "@dinghy/base-components";
import { DATA_LOSS_PREVENTION_API } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, DATA_LOSS_PREVENTION_API)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 32,
}
```

  </TabItem>
</Tabs>


## Managed MS Ad

<Tabs>
  <TabItem value="Preview">

![](./managed-ms-ad.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { ManagedMsAd } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<ManagedMsAd/>

// or

import { extendStyle } from "@dinghy/base-components";
import { MANAGED_MS_AD } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, MANAGED_MS_AD)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 37,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Secret
Manager

<Tabs>
  <TabItem value="Preview">

![](./secret-manager.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { SecretManager } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<SecretManager/>

// or

import { extendStyle } from "@dinghy/base-components";
import { SECRET_MANAGER } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, SECRET_MANAGER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 25,
}
```

  </TabItem>
</Tabs>


## Security
Command Center

<Tabs>
  <TabItem value="Preview">

![](./security-command-center.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { SecurityCommandCenter } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<SecurityCommandCenter/>

// or

import { extendStyle } from "@dinghy/base-components";
import { SECURITY_COMMAND_CENTER } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, SECURITY_COMMAND_CENTER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 36,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Security Key
Enforcement

<Tabs>
  <TabItem value="Preview">

![](./security-key-enforcement.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { SecurityKeyEnforcement } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<SecurityKeyEnforcement/>

// or

import { extendStyle } from "@dinghy/base-components";
import { SECURITY_KEY_ENFORCEMENT } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, SECURITY_KEY_ENFORCEMENT)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 34,
  _height: 42,
}
```

  </TabItem>
</Tabs>


## Web Security
Scanner

<Tabs>
  <TabItem value="Preview">

![](./web-security-scanner.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { WebSecurityScanner } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<WebSecurityScanner/>

// or

import { extendStyle } from "@dinghy/base-components";
import { WEB_SECURITY_SCANNER } from '@dinghy/diagrams/entitiesGcp2IconsSecurity'

<Shape 
  {...props}
  _style={extendStyle(props, WEB_SECURITY_SCANNER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 36,
}
```

  </TabItem>
</Tabs>

