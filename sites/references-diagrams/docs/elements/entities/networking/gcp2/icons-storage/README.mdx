import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

# Gcp2 Storage Entities


## Cloud
Storage

<Tabs>
  <TabItem value="Preview">

![](./cloud-storage.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { CloudStorage } from '@dinghy/diagrams/entitiesGcp2IconsStorage'

<CloudStorage/>

// or

import { extendStyle } from "@dinghy/base-components";
import { CLOUD_STORAGE } from '@dinghy/diagrams/entitiesGcp2IconsStorage'

<Shape 
  {...props}
  _style={extendStyle(props, CLOUD_STORAGE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 34,
}
```

  </TabItem>
</Tabs>


## Data Transfer

<Tabs>
  <TabItem value="Preview">

![](./data-transfer.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { DataTransfer } from '@dinghy/diagrams/entitiesGcp2IconsStorage'

<DataTransfer/>

// or

import { extendStyle } from "@dinghy/base-components";
import { DATA_TRANSFER } from '@dinghy/diagrams/entitiesGcp2IconsStorage'

<Shape 
  {...props}
  _style={extendStyle(props, DATA_TRANSFER)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 35,
}
```

  </TabItem>
</Tabs>


## Filestore

<Tabs>
  <TabItem value="Preview">

![](./filestore.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { Filestore } from '@dinghy/diagrams/entitiesGcp2IconsStorage'

<Filestore/>

// or

import { extendStyle } from "@dinghy/base-components";
import { FILESTORE } from '@dinghy/diagrams/entitiesGcp2IconsStorage'

<Shape 
  {...props}
  _style={extendStyle(props, FILESTORE)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 42,
  _height: 34,
}
```

  </TabItem>
</Tabs>


## Persistent
Disk

<Tabs>
  <TabItem value="Preview">

![](./persistent-disk.png#drawio)

  </TabItem>
  <TabItem value="Usage">
    
```js
import { PersistentDisk } from '@dinghy/diagrams/entitiesGcp2IconsStorage'

<PersistentDisk/>

// or

import { extendStyle } from "@dinghy/base-components";
import { PERSISTENT_DISK } from '@dinghy/diagrams/entitiesGcp2IconsStorage'

<Shape 
  {...props}
  _style={extendStyle(props, PERSISTENT_DISK)}
  />

```

  </TabItem>
  <TabItem value="Definition">
    
```js
{
  _style: {
    entity: 'sketch=0;html=1;verticalAlign=top;labelPosition=center;verticalLabelPosition=bottom;align=center;spacingTop=-6;fontSize=11;fontStyle=1;fontColor=#999999;shape=image;aspect=fixed;imageAspect=0;image=data:image/svg+xml,;strokeColor=none;',
  },
  _width: 34,
  _height: 42,
}
```

  </TabItem>
</Tabs>

