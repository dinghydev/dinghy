...
`dinghy.config.yml`. This is typically the first place you will customize how
...
By default, Dinghy uses the current working directory as the application's home directory. If you want Dinghy to treat a different location as your project root, set the `APP_HOME` environment variable to your desired path. Dinghy will then operate from that directory as the application's root.
...
larger/complex project while keep top level `dinghy.config.yml` managable.

### Stack Discovery

There are two way to define a stack:

1. Define your avaiable stacks as top level object in `dinghy.config.yml`.
...
1. foo-bar-baz-*.yml (any file starting with the full name plus `-`, sorted alphabetically — highest priority)
1. foo-bar-baz.yml
1. bar-baz.yml
1. foo-bar.yml
1. baz.yml
...
The `foo-bar-baz-*.yml` prefix match lets you split per-stack overrides across
multiple files (e.g. one file per concern) without touching the base
`foo-bar-baz.yml`. Files are sorted alphabetically, then deep-merged on top of
all tag-matched files.
...
export const renderOptions = (options: RenderOptions){
    options.stack.title=`Enviroment: ${options.stack.name}`
    if(options.stack.name==='dev'){
        options.stack.configUrls={
          'inputs.mysolutionstack.output': 'https://my-solution-network-backend.s3.us-east-1.amazonaws.com/output/stack.yml'
        }
    }
}
```
...
1. S3 object URLs (e.g.,
   `https://my-bucket.s3.us-west-2.amazonaws.com/my-folder/config.yml`) can
...
After a double dash (`--`), Dinghy passes all subsequent arguments directly to the underlying tool without interpreting them. This is helpful if the arguments you need to pass would otherwise be processed by Dinghy itself.

For example, running `dinghy aws s3 --debug` will enable Dinghy's debug mode, because `--debug` is parsed by Dinghy.

If you want to pass `--debug` (or any other option) to the AWS CLI instead of Dinghy, use a double dash:  
`dinghy aws s3 -- --debug`  
In this case, everything after `--` (here, `--debug`) will be forwarded unmodified to `awscli`.
