...
| `--arch`       | host arch (or `DINGHY_DOCKER_SUPPORTED_ARCHS`)        | Architectures to build (multiple values)              |
...
│   └── postbuild.sh                # optional: runs after docker build (always)
├── 20-app/
│   └── Dockerfile
└── 30-worker/
    └── Dockerfile
...
enable it — `--multi-arch` then defaults to `true` and `--arch` defaults to
that list. When the variable is unset or `false`, builds are single-arch for
the host only.
...
| `DINGHY_DOCKER_SUPPORTED_ARCHS` | Comma-separated list of arches; presence enables multi-arch by default          |
...
not belong in the Dockerfile itself. `postbuild.sh` runs even when the build
fails, so it is the right place to undo any temporary edits made by
`prebuild.sh`.
