...
slides/
  ...
  my-talk/            ← one folder = one presentation
    ...
    *.yml / *.md / *.html / *.css  ← auto-loaded, sorted by filename
...
sections:
  - badge: Hello
    title: My First Presentation
    p: 'Author in YAML — output self-contained RevealJS HTML.'
...
- [dinghy slide serve](../../references/commands/engine/slide/serve) Serves the
...
Individual `.yml` (excluding `slide.yml`), `.md`, `.html`, and `.css` files
...
| `title`   | Optional HTML title shown centered over the region        |
...
### Key RevealJS config options

| Key           | Description                               | Default |
| ------------- | ----------------------------------------- | ------- |
| `theme`       | `black`, `white`, `moon`, `solarized`, …  | `black` |
| `transition`  | `none`, `fade`, `slide`, `convex`, `zoom` | `slide` |
| `slideNumber` | e.g. `"c/t"` for current / total          |         |
...
The demo presentation is authored entirely in YAML
[slide.yml](https://github.com/dinghydev/dinghy/blob/main/sites/www/slides/revealjs-features-demo/slide.yml):

<div style={{'max-height': '300px','overflow-y': 'auto'}}>
<CodeBlock language="tsx" title="sites/www/slides/revealjs-features-demo/slide.yml">
  {DemoYml}
</CodeBlock>
</div>
